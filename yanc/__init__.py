...
def check_nc_file_against_template(ncfile, template, debug):
    ...
    with open(template, "r") as s:
        try:
            check_params = yaml.load(s, Loader=yaml.FullLoader)
        ...
        for dimension in check_params["dimensions"]:
            ...
            if "length" in dimension:
                if val != dimension["length"]:
                    print(
                        "Dimension length ({}) for  '{}' is not equal to {}".format(
                            len(dataset.dimensions[name]), name, dimension["length"]
                        )
                    )
                    return NOT_OK
            if "min_length" in dimension:
                if val < dimension["min_length"]:
                    print(
                        "Dimension length ({}) for  '{}' is less than {}".format(
                            len(dataset.dimensions[name]), name, dimension["min_length"]
                        )
                    )
                    return NOT_OK
            if "max_length" in dimension:
                if val > dimension["max_length"]:
                    print(
                        "Dimension length ({}) for  '{}' is greater than {}".format(
                            len(dataset.dimensions[name]), name, dimension["max_length"]
                        )
                    )
                    return NOT_OK
    ...
    if "variables" in check_params:
        for variable in check_params["variables"]:
            ...
            if (
                allowed_missing_percent == 100
                and "min" not in variable
                and "max" not in variable
            ):
                if debug:
            ...
            if len(values.shape) > 0:
                ...
                if "missing_timesteps" in variable:
                    ...
                    timesteps = [
                        t for t in timesteps if t not in allowed_missing_timesteps
                    ]
            ...
            if val > allowed_missing_percent:
                print(
                    "Missing values ({}%) for '{}' is higher than {}%".format(
                        val, name, allowed_missing_percent
                    )
                )
                return NOT_OK
            ...
            if "min" in variable:
                ...
                if variable["min"] > val:
                    print(
                        "Minimum value ({}) for '{}' is lower than {}".format(
            ...
            if "max" in variable:
                ...
                if variable["max"] < val:
                    print(
                        "Maximum value ({}) for '{}' is higher than {}".format(
...
def run(args):
    ...
    parser.add_argument(
        "--ncfile", required=True, help="Name of the input NetCDF file."
    )
    parser.add_argument(
        "--template", required=True, help="Name of the file with parametrized checks."
    )
